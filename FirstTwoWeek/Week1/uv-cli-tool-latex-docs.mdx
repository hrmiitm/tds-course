...
- Completed [Lab 1.1](./publish-python-library-pypi-uv.mdx) (at least through Step 6) — you understand UV + pyproject.toml.
- LaTeX + pandoc available locally ([see latex.mdx](../../week-1/latex.mdx)). GitHub Actions has both preinstalled — so local is optional.
...
$$
\text{output} = \text{Render}\big(\text{SortBy}_{\text{col}}(\text{Read}(\text{csv}))[:N]\big)
$$
...
[**Lab 1.3 — Bash automation: daily project summary**](./bash-daily-project-summary.mdx)
